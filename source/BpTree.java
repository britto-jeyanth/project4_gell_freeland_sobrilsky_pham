       ...
       implements Serializable, Cloneable, SortedMap <K, V>
{
     ...
     * @author Ryan Gell & Minh Pham
     */
    @SuppressWarnings("unchecked")
    public SortedMap <K,V> headMap (K toKey)
    {
	BpTree<K, V> map = new BpTree<K, V>(classK, classV);
        Node n=root;
        Queue<Node> myQueue = new LinkedList<Node>();
        myQueue.add(n);
        while(!myQueue.isEmpty()){
               ...
               if(temp.isLeaf){
        		for(int i=0; i<temp.nKeys; i++){
				if(temp.key[i].compareTo(toKey) < 0){
        				map.put(temp.key[i], (V) temp.ref[i]);
				}
        		}
        	}
    ...
    public SortedMap <K,V> tailMap (K fromKey)
    {
        ...
        while(!myQueue.isEmpty()){
               ...
               if(temp.isLeaf){
        		for(int i=0; i<temp.nKeys; i++){
				if(temp.key[i].compareTo(fromKey) >= 0){
        				map.put(temp.key[i], (V) temp.ref[i]);
				}
        		}
        	}
	...
	BpTree<K, V> map = new BpTree<K, V>(classK, classV);
        ...
        while(!myQueue.isEmpty()){
               ...
               if(temp.isLeaf){
        		for(int i=0; i<temp.nKeys; i++){
				if(temp.key[i].compareTo(fromKey) >= 0 && temp.key[i].compareTo(toKey) < 0){
        				map.put(temp.key[i], (V) temp.ref[i]);
				}
        		}
        	}
